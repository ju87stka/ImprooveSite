...
if (!$stmt = mysqli_prepare($link, $sql)) {
    ...
    echo "проблема";
     ...
     mysqli_stmt_bind_param($stmt, "ssii", $title, $author, $pubyear, $price); mysqli_stmt_execute($stmt);
...
function myBasket(){
    ...
    $ids = implode(",", $goods); 
    $sql = "SELECT id, author, title, pubyear, price           FROM catalog WHERE id IN ($ids)"; if(!$result = mysqli_query($link, $sql))   
        return false;
...
function getOrders(){
    ...
    foreach ($orders as $order) { 
        list($name, $email, $phone, $address,
            $date, $orderid ) = explode(";", trim($order)); 
